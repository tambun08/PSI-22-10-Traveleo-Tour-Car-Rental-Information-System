...
class BookingController extends Controller
{
    ...
    {
        $data = Pesanan::with('user:id,name', 'cars:id,nama_mobil,merk_mobil')
        ->where('status_peminjaman', 1)
        ->where('status_pembayaran', 4)
        ->orWhere('status_pembayaran', 5)
        ->paginate(10);
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'sewa_supir' => 'required|string|max:255',
        ...
        $data->sewa_supir = $request->sewa_supir;
